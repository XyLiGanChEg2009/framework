class Graph3DUI extends Component {
    ...
    lightHandler() {
        this.callbacks.powerOfLight(document.querySelector('.lightPower').value - 0);
    }

    menuHandler() {
        document.querySelector('.container3D').classList.toggle('containerActive');
        if (this.menuOpen) {
            setTimeout(() => document.querySelector('.block3D').classList.toggle('hide'), 1000);
            document.getElementById('figuresList').classList.add('hide');
        } else {
            setTimeout(() => document.querySelector('.block3D').classList.toggle('hide'), 0);
        }
    ...
    addFigureHandler(event) {
        ...
        document.getElementById('figuresList').classList.add('hide');
        this.callbacks.selectFigure(figure, this.num);
    ...
    changeFigureSettigHandler(event) {
        ...
        this.callbacks.changeFigureSettings(
            elem.dataset.num,
    ...
    createInput(type, dataset, placeholder, className = "FiguresSettingsInputs") {
